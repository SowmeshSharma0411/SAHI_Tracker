...
# Load the SAHI detection model with a slightly increased confidence threshold
detection_model = AutoDetectionModel.from_pretrained(
    model_type="yolov8",
    model_path="best.pt",
    confidence_threshold=0.25,  # Adjust for fewer detections
    device="cuda"
)
...
def frame_reader():
    while cap.isOpened():
        ...
        if not success:
            break
        resized_frame = cv2.resize(frame, (1037, 583), interpolation=cv2.INTER_AREA)
        raw_frame_queue.put(resized_frame)  # Keep original frame size
...
def sahi_detector():
    while True:
        ...
        # Perform slicing with original size frames
        sliced_results = get_sliced_prediction(
            image=frame,
            detection_model=detection_model,
            slice_height=315,  # Adjust as needed
            slice_width=560,
            overlap_height_ratio=0.25,  # Adjust overlap to minimize redundant detections
